class Grid {
  ...
  void endTurn() {
    for (int i = 0; i < tetronimo.shape.matrix.length; ++i)
      for (int j = 0; j < tetronimo.shape.matrix.length; ++j)
        if (tetronimo.shape.matrix[i][j] && j + tetronimo.y >= 0)
          colors[i + tetronimo.x][j + tetronimo.y] = tetronimo.getColor();
    if (checkLines()) {
      tetronimo = null;
      animateCount = 0;
    } else
